...
fn parse_grid(input: Span) -> IResult<Span, HashMap<IVec2, u32>> {
    let (input, output) = separated_list1(line_ending, many1(a_num))(input)?;
    let map: HashMap<IVec2, u32> = output.into_iter().flat_map(|v| v.into_iter()).collect();
    Ok((input, map))
}
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let (_, map) = parse_grid(Span::new(input)).expect("Should parse");
    let boundaries = IVec2::new(columns, rows);
    let goal = IVec2::new(columns - 1, rows - 1);

    let result: (Vec<(IVec2, VecDeque<IVec2>)>, u32) = dijkstra(
        ...
        |(position, deque)| {
            ...
            options
                ...
                .filter_map(|pos_diff| {
                    ...
                    if (0..boundaries.x).contains(&next_position.x)
                        && (0..boundaries.y).contains(&next_position.y)
                    {
                ...
                .map(|pos| {
                    let next_cost = *map.get(&pos.0).unwrap();
                    (pos, next_cost)
                })
