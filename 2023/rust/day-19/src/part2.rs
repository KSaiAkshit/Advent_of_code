        ...
        match self {
            Rule::Test {
                ...
                if test_value.contains(value) {
                    ...
                    match condition {
                        Condition::LessThan => {
                            let new_range_low = *test_value.start()..=(value - 1);
                            let mut part_low = part.clone();
                            part_low.set_field(part_field, new_range_low);

                            let new_range_high = *value..=*test_value.end();
                            let mut part_high = part.clone();
                            part_high.set_field(part_field, new_range_high);
                            Apply::Split {
                        ...
                        Condition::GreaterThan => {
                            let new_range_low = *test_value.start()..=*value;
                            let mut part_low = part.clone();
                            part_low.set_field(part_field, new_range_low);

                            let new_range_high = (*value + 1)..=*test_value.end();
                            let mut part_high = part.clone();
                            part_high.set_field(part_field, new_range_high);
                            Apply::Split {
                    ...
                    // only do one target
                    if (test_value.end() < value && cond == Ordering::Less)
                        || (test_value.start() > value && cond == Ordering::Greater)
                    {
...
impl Part {
    fn set_field(&mut self, field: &str, new_range: RangeInclusive<u64>) {
        match field {
            "x" => {
                self.x = new_range;
            }
...
pub fn parse(input: &str) -> IResult<&str, HashMap<&str, Workflow>> {
    ...
    match next_target {
        Target::Workflow(id) => {
            let current_workflow = workflows
                .get(id)
                .expect("should always produce a valid workflow given a valid id");
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let (_input, workflows) = parse(input).expect("should parse");
