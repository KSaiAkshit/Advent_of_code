...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let col_count = input.lines().next().unwrap().len() as i32;
    let _bounds = IVec2::new(row_count, col_count);
    let (_input, grid) = parse_grid(Span::new(input)).expect("Should parse");
    let grid_map = grid
        .iter()
    ...
    let start_position = grid.iter().find(|plot| plot.span.extra.y == 0).unwrap();
    ...
    grid.iter()
        .flat_map(|plot| {
            let possible_directions = match plot.r#type {
                PlotType::Directional(direction) => vec![direction],
                PlotType::Empty => vec![
                    Direction::North,
                    Direction::South,
                    Direction::East,
