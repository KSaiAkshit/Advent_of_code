...
impl Hail {
    ...
    fn solve_intersection(&self, hail_two: &Hail) -> Result<(f64, f64, DVec2), LinalgError> {
        use ndarray::prelude::*;
        use ndarray_linalg::Solve;

        let a: Array2<f64> = array![
            [self.direction.x as f64, -hail_two.direction.x as f64],
            [self.direction.y as f64, -hail_two.direction.y as f64],
        ];
        let b: Array1<f64> = array![
            hail_two.starting_position.x as f64 - self.starting_position.x as f64,
            hail_two.starting_position.y as f64 - self.starting_position.y as f64
        ];
...
fn parse(input: &str) -> IResult<&str, Vec<Hail>> {
    separated_list1(
        line_ending,
        separated_pair(ivec3, delimited(space1, tag("@"), space1), ivec3).map(
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let results = hail
        ...
        .filter(|(_, (xx, yy, result))| {
            bound.contains(&result.x) && bound.contains(&result.y) && xx >= &0. && yy >= &0.
        })
...
fn solve_intersection() {
    ...
    let h1 = Hailf64 {
        starting_position: DVec3::new(232488932265751., 93844132799095., 203172424390144.),
        direction: DVec3::new(64., 273., 119.),
    };
