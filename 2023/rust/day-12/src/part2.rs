...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let puzzles = input
        .lines()
        .map(parse_line)
        .collect::<Result<Vec<(&str, Puzzle)>, nom::Err<nom::error::Error<&str>>>>()
...
fn parse_line(input: &str) -> IResult<&str, Puzzle> {
    ...
    let spaces_to_fill = expanded_line.chars().filter(|c| c == &'?').count() as u32;
    ...
    Ok((
        ...
        Puzzle {
            spaces_to_fill,
            line: expanded_line,
            batches: std::iter::repeat(batches).take(5).flatten().collect(),
        },
    ))
}
...
impl Puzzle {
    ...
    fn generate_permutations(&self) -> impl Iterator<Item = String> {
        repeat_n([".", "#"].into_iter(), self.spaces_to_fill as usize)
