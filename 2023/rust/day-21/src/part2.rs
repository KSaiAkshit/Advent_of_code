...
fn parse_rock_grid(input: Span) -> IResult<Span, (HashSet<IVec2>, IVec2)> {
    fold_many1(
...
pub fn process(input: &str, step_count: usize) -> miette::Result<String, AocError> {
    let (_, (set, start)) = parse_grid(Span::new(input)).expect("a valid parse");
    let (_, (_rock_set, _)) = parse_rock_grid(Span::new(input)).expect("a valid second parse");
    ...
    .inspect(|(i, set)| {
        ...
        let [c, b, a] =
            (vandermonde.inverse() * Vec3::new(seq0 as f32, seq1 as f32, seq2 as f32)).to_array();
        let a = a as i64;
        let b = b as i64;
        let c = c as i64;
    ...
    for y in y_bound {
        for x in x_bound.clone() {
            match (
                ...
                rocks.contains(&(IVec2::new(x, y).rem_euclid(rock_bounds))),
            ) {
                (true, _) => {
                    print!("O");
                }
...
.##..##.##.
...........";
        assert_eq!(expected_tiles.to_string(), process(input, step_count)?);
