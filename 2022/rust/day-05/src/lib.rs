...
fn parse_crate(input: &str) -> IResult<&str, Option<&str>> {
    // Parse each "Line" and check whether it has "   "(three spaces) or "[X]"(A crate surrounded by []).
    // Only useful for the first match
    let (input, c) = alt((
        tag("   "),
...
fn line(input: &str) -> IResult<&str, Vec<Option<&str>>> {
    // Parse the line and remove all spaces. Store each value in a Vec of options
...
fn move_crate(input: &str) -> IResult<&str, Move> {
    // Parses the Instructions and removes unnecassary characters
